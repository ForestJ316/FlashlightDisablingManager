...
void DisableEffectHandler::Initialize()
{
	logger::info("Initializing DisableEffectHandler...");
	...
	if (dataHandler) {
		...
		if (!FlashlightEffectID) {
			logger::critical("Failed to initialize FDM_Flashlight Disable Effect (EditorID: FDM_FlashlightEffect)");
		}
	}
	...
	logger::info("...DisableEffectHandler initialized.");
}

void DisableEffectHandler::OnEffectStart(RE::ActiveEffect* a_effect)
{
	...
	if (a_effect->effect->effectSetting == FlashlightEffectID && a_effect->target->GetTargetStatsObject() == RE::PlayerCharacter::GetSingleton()) {
		...
		if (!UI->GetMenuOpen("LoadingMenu"sv) && !UI->GetMenuOpen("FaderMenu"sv)) {
			...
			DisableEffectHandler::GetSingleton()->sActionToDoAfterMenu = "Off";
		}
	}
}

void DisableEffectHandler::OnEffectFinish(RE::ActiveEffect* a_effect)
{
	_OnEffectFinish(a_effect);

	if (a_effect->effect->effectSetting == FlashlightEffectID && a_effect->target->GetTargetStatsObject() == RE::PlayerCharacter::GetSingleton()) {
		auto effectHandler = DisableEffectHandler::GetSingleton();
		...
		if (effectHandler->iActiveFlashlightEffectCount > 0) {
			...
			if (effectHandler->iActiveFlashlightEffectCount == 0 && !RE::PlayerCharacter::GetSingleton()->IsDead(true)) {
				...
				if (!UI->GetMenuOpen("LoadingMenu"sv) && !UI->GetMenuOpen("FaderMenu"sv)) {
					...
					effectHandler->sActionToDoAfterMenu = "On";
				}
			}	
		}
	}
...
void DisableEffectHandler::OnEffectFinishLoadGame(RE::ActiveEffect* a_effect)
{
...
DisableEffectHandler::FaderMenuEvent::EventResult DisableEffectHandler::FaderMenuEvent::ProcessEvent(const RE::MenuOpenCloseEvent& a_event, RE::BSTEventSource<RE::MenuOpenCloseEvent>*)
{
	if (!a_event.opening && a_event.menuName == "FaderMenu"sv && DisableEffectHandler::GetSingleton()->sActionToDoAfterMenu != "") {
		std::string& sAction = DisableEffectHandler::GetSingleton()->sActionToDoAfterMenu;
		if (sAction == "On") {
			FlashlightHandler::GetSingleton()->TurnOnFlashlight();
		}
		else if (sAction == "Off") {
